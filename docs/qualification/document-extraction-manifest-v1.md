...
## Merged D3 native-text qualification record

The following record is retained from the current public `master` head and is
kept as a separate bounded producer qualification record. It does not replace
the current-head continuity and paired seam observations above.
...
The retained run was performed on 2026-07-23 from MinerU qualification branch
head `9301f637d01317351322f3fac9b7c3155e5b6ced`, based on live default head
`72fcb4a5d688b2d23acd619f3de2f2bd85c44775`, using local model snapshot
`ed6b654c018d742e65a17671e379c5e6ecc87ec9` in image
`sha256:e034f798206a8cdd384a6c3986693cbfe385fe2ed585952963eaeac84ec836c4`.
The source checkout was mounted read-only into the local-model container with
an internal `--network none` network, no host ports, no provider, no private
data, and no VLM/OCR path.
...
evidence at `E:\Codex\30-evidence\ra-recovery-program-v1-0\d3-native-text-20260723-r3`.
...
The executable implementation is `mineru/qualification/native_text.py`; the
operator entry point is `tools/qualify_native_text.py`. Generated fixtures and
output are not repository artifacts.

### Remaining D3 gates

This record does not qualify OCR, hybrid/VLM, rotated pages, long documents,
multilingual or Traditional Chinese coverage, encrypted/corrupt inputs,
duplicate/superseded source handling, host-level egress policy, or
...
private-data processing, or runtime release. Those remain separate gates.
